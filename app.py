...
def test():
    if not request.json or not "HELLO WORLD" in request.json:
        abort(400)
    db.post("/test", {"HELLO WORLD": request.json["HELLO WORLD"]})
...
#            abort(400)
        ...
        if "www.multigp.com" not in url:
            url = "http://www.multigp.com/races/view/{}".format(url)
...
def check_event_exists(url):
    ...
    if eventId:
        ...
        events = db.get("/races/view/{}".format(url), None)
        if events:
            if "id" in events:
                eventId = events["id"]
            else:
                for title in events:
                    eventId = events[title]["id"]
        if eventId:
            return eventId
        else:
            return None
...
# Complete updates race structure after receiving new structure from finished race rebuilding wizard
@app.route("/rebuild/race/<path:url>", methods=["POST"])
def rebuild_race_wizard(url):
    if not request.json:
        abort(400)
    eventId = db.get(url, None)
    eventId = eventId["id"]
    db.patch("/events/{}".format(eventId), request.json)
    return jsonify({"result": "who knows dude"})

# Updates attendance for an event from multigp
@app.route("/update/race/attendance/multigp", methods=["POST"])
def update_attendance_from_multigp():
    if not request.form:
        abort(400)
    newRacers = scraper.scrapeAttendance(request.form["url"])
    eventId = db.get("/{}".format(request.form["url"].strip("http://www.multigp.com/")), None)["id"]
    db.put("/events/{}".format(eventId), "racers", newRacers)
    return jsonify({"result": "success"})
